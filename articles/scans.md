...
scans analyzes completed agent records without calling a model or
running a tool. This article introduces trajectories, follows a
completed run to a finding, and explains the built-in scans.
...
- **Events** record text content, tool calls, tool results, thinking,
  errors, and source-specific records. Every event has an identifier, a
  position in canonical order, and optional timing, usage, and status.
  An event may belong to a turn or directly to a trajectory.
...
A **finding** is a detector result with references to the supporting
trajectory, turn, or events.
...
activity, unresolved calls, failures, token usage, and duration, for
sorting and filtering runs before opening a transcript:
...
runs the built-in detectors and returns their findings:
...
stable for the same bundle, scan identifier, and threshold settings.
...
| `error_chain` | At least two failed events are connected by `parent_event_id` links. Earlier unrelated failures do not form a chain. |
...
store. Records append to a JSON-lines log; a revised judgment retains
earlier records. For deployment, use shared persistent storage and a
single app process as described in
