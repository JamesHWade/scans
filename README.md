...
An evaluation score says whether an agent produced the right outcome. scans
explains the path that produced it. It turns completed agent runs into
evidence-linked trajectories that you can summarize, scan for common failure
patterns, and inspect in a read-only review app, without calling the model or
its tools again.

Reach for scans when a pass/fail score is not enough: a tool failed somewhere
in a long run, an agent repeated the same request, token usage jumped, or a
...
The core analysis layer depends only on cli, rlang, S7, and tibble. Adapters
...
finding per problem, each carrying the trajectory and event identifiers that
support it. `scans_app()` opens the same bundle so you can move from a finding
to the transcript event behind it.
...
- **Record judgements** by passing a `scans_annotations()` store to the app.
  Labels and notes append to a JSON-lines log that never rewrites earlier
  records, so reviewers cannot overwrite each other and every record stays
  available to your own analysis.
