...
* `scans_app()` launches a read-only Shiny app for filtering trajectories, browsing canonical turns and events, and inspecting linked findings, evaluations, losses, and source context. Named bundle or loader lists add a lazy, reloadable application switcher suited to reviewing multiple deployments on Posit Connect (#2).
